...
type LogFn = (callerName: string, message: string, additionalData?: Record<string, any>) => void;
...
export class Logger {
  ...
  constructor(config: LoggerConfig) {
    ...
    this.verbose = makeLogFn(config, "verbose");
  ...
  setContext(context: Record<string, string>) {
    ...
    this.verbose = makeLogFn(nextConfig, "verbose");
...
function makeLogFn(config: LoggerConfig, level: LogLevel): LogFn {
  ...
  const consoleLevel = level === "verbose" ? "debug" : level;
  ...
  if (config.structuredOutput) {
    ...
    return (callerName, message, ...additionalData) => {
      // eslint-disable-next-line no-console
      console[consoleLevel](`${prefix} ${callerName}: ${message}`, ...additionalData);
    };
  }
}
