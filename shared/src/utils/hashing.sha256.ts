 ...
 */
export async function sha256(input: string): Promise<Sha256String> {
  if (crypto.subtle === undefined) {
    if (!hasWarnedAboutInsecureContext) {
      // eslint-disable-next-line no-console
      console.warn(
        "Direct.dev: You're running in an insecure context, which prevents using crypto.subtle for sha256 hashing. Your application may work just fine, but risk of cache collisions increase sharply.",
      );
    }
    ...
    return input.slice(0, 37) as Sha256String;
  }
  ...
  return base128 as Sha256String;
}
