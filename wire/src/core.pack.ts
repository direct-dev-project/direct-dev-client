...
export const unpack = {
  ...
  arr<T>(input: string, cursor: number, unpack: (cursor: number) => [T, number]): [T[], number] {
    ...
    const result: T[] = new Array(Math.min(len, 256));
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (value[1] <= cursor) {
        throw new Error("[DIRECT] unpack.arr(): invalid unpacking of array, next cursor must be incremented");
      }
...
export function makeEnumPacker<T extends string | number | boolean | null | undefined>(
  ...
  const encodeMap = new Map(values.map((value, index) => [value, String.fromCharCode(index)]));
  const decodeMap = new Map(values.map((value, index) => [index, value]));

  return {
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    encode: (input: T) => encodeMap.get(input)!,
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    decode: (input, cursor) => [decodeMap.get(input.charCodeAt(cursor))!, cursor + 1],
  };
}
...
export const WIRE_DICTIONARY = [
  ...
  "invalid request signature (Direct.dev)",
  "request method not supported (Direct.dev)",
  "failed to receive response (Direct.dev)",
  "an internal server error occurred (Direct.dev)",
